<template>
    ...
    <div>
        ...
        <el-table :data="tableData" ref="multipleTable" border stripe style="width:100%;" @selection-change="selectInfo">
            <el-table-column fixed header-align="center" align="center" type="selection" width="45" scope.row.id prop="id" @selection-change="selectInfo" />
            ...
            <div v-for="(item,index) in filterList" :key="index" >
                <el-table-column label="公司名称" prop="pName" fixed v-if="item.prop == 'pName' && item.state == 1" min-width="200" sortable>
                    <template slot-scope="scope">
                ...
                <el-table-column label="联系人" prop="contacts[0].coName" v-else-if="item.prop == 'contacts[0].coName' && item.state == 1" min-width="100" sortable />
                <el-table-column label="电话" prop="contacts[0].telephone" v-else-if="item.prop == 'contacts[0].telephone' && item.state == 1" min-width="100" sortable />
                <el-table-column label="手机" prop="contacts[0].phone" v-else-if="item.prop == 'contacts[0].phone' && item.state == 1" min-width="110" sortable />
                <el-table-column label="QQ" prop="contacts[0].qq" v-else-if="item.prop == 'contacts[0].qq' && item.state == 1" min-width="90" sortable />
                ...
                <el-table-column label="负责人" prop="privateUser[0].private_employee" v-else-if="item.prop == 'privateUser[0].private_employee' && item.state == 1" min-width="100" sortable />
                <el-table-column label="部门" prop="deptname" v-else-if="item.prop == 'deptname' && item.state == 1" min-width="100" sortable />
                <el-table-column label="机构" prop="parentname" v-else-if="item.prop == 'parentname' && item.state == 1" min-width="100" sortable />
                ...
                <el-table-column label="状态" prop="status" v-else-if="item.prop == 'status' && item.state == 1" min-width="100" sortable />
                <el-table-column label="客户分类" prop="levels" v-else-if="item.prop == 'levels' && item.state == 1" min-width="110" sortable />
                <el-table-column label="客户来源" prop="source" v-else-if="item.prop == 'source' && item.state == 1" min-width="110" sortable />
                <el-table-column label="法人代表" prop="representative" v-else-if="item.prop == 'representative' && item.state == 1" min-width="110" sortable />
                <el-table-column label="登记机关" prop="registrationAuthority" show-overflow-tooltip v-else-if="item.prop == 'registrationAuthority' && item.state == 1" min-width="150" sortable />
                <el-table-column label="社会信用代码" prop="creditCode" v-else-if="item.prop == 'creditCode' && item.state == 1" min-width="160" sortable />
...
<script>
    ...
    export default {
        ...
        beforeRouteLeave(to, from , next){
            ...
            bus.$emit('collapse4', this.collapse4);
            next()
        },
        beforeCreate(){
            const _this = this
        ...
        methods: {
            ...
            search() {
                ...
                let i = 1
                if(this.searchList.label == 0 ){
                    authorityInterface = 'customerJurisdiction/all.do'//全部客户
                    i = 0
                }else if(this.searchList.label == 2){
                    authorityInterface = 'customerJurisdiction/second.do'//本组客户
                    i = 0
                }else if(this.searchList.label == 3){
                    authorityInterface = 'customerJurisdiction/dept.do'//本机构客户
                    i = 0
                }
            ...
            generateCharts(){
                ...
                axios({
                    method: 'post',
                    url: _this.$store.state.defaultHttp+'customerpool/selectByStateAndSourceAndType.do?cId='+_this.$store.state.iscId,
                    data: qs.stringify(data)
                }).then(function(res){
                    if(res.data.code && res.data.code == '200'){
