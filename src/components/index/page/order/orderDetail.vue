    ...
    <div class="orderadd_c">
            ...
            <div class="orderHead">
                <el-form :inline="true" ref="myform" :model="myform" :rules="rules">
                    ...
                    <el-form-item class="first_input" label="联系人" label-width="90px">
                        <el-select v-model="myform.contactId" :disabled="myform.checkStatus !== 0" placeholder="请选择联系人" class="inputbox">
                    ...
                    <el-form-item prop="orderTime" class="first_input" label="订单时间" label-width="90px">
                        <el-date-picker v-model="myform.orderTime" :disabled="myform.checkStatus !== 0" type="date" placeholder="选择订单时间" format="yyyy-MM-dd" value-format="yyyy-MM-dd" class="inputbox"></el-date-picker>
                    </el-form-item>
                    <el-form-item prop="settlement" class="first_input" label="结算方式" label-width="90px">
                        <el-select v-model="myform.settlement" :disabled="myform.checkStatus !== 0" placeholder="请选择结算方式" class="inputbox">
                    ...
                    <el-form-item class="first_input" label="交货方式" label-width="90px">
                        <el-select v-model="myform.deliveryMode" :disabled="myform.checkStatus !== 0" placeholder="请选择交货方式" class="inputbox">
        ...
        <el-table v-loading="listLoading" :data="itemData" border fit highlight-current-row show-summary :summary-method="getSummary" @cell-click="cellClick" style="width: 100%">
            ...
            <div v-for="(item,index) in filterList" :key="index" >
                <el-table-column label="产品名称" prop="tbGoods.goodsName" width="280px" v-if="item.prop == 'goodsName' && item.state == 1" class="table_required">
                ...
                <el-table-column label="金额" prop="amountOfMoney" min-width="120" v-if="item.prop == 'amountOfMoney' && item.state == 1"></el-table-column>

                <el-table-column label="折扣" prop="discount" width="90" v-if="item.prop == 'discount' && item.state == 1">
                    <template slot-scope="scope">
                        <template v-if="scope.row.edit">
                            <el-input v-model="scope.row.discount" class="edit-input" size="small" @input="handleinput($event,scope.$index,scope.row,'discount')">
                ...
                <el-table-column label="折扣额" prop="discountAmount" min-width="120" v-if="item.prop == 'discountAmount' && item.state == 1">
                    <template slot-scope="scope">
                        <template v-if="scope.row.edit">
                            <el-input v-model="scope.row.discountAmount" class="edit-input" size="small" @input="handleinput($event,scope.$index,scope.row,'discountAmount')"/>
                        </template>
                        <span v-else>{{ scope.row.discountAmount }}</span>
                    </template>
                </el-table-column>

                <el-table-column label="折后金额" prop="discountAfter" min-width="120" v-if="item.prop == 'discountAfter' && item.state == 1"></el-table-column>
                ...
                <el-table-column label="税额" prop="taxAmount" min-width="120" v-if="item.prop == 'taxAmount' && item.state == 1"></el-table-column>

                <el-table-column label="税后金额" prop="taxAfter" min-width="120" v-if="item.prop == 'taxAfter' && item.state == 1"></el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            loadData(){
                const _this = this
                let orderdetailsData = this.$store.state.orderdetailsData
                this.orderdetailsData = orderdetailsData
                let qs = require('querystring')
                let data = {}
                data.id = orderdetailsData.id
                ...
                axios({
                    ...
                    if(res.data.checkStatus == 0){
                        _this.$options.methods.loadfield.bind(_this)()
                        _this.$options.methods.loadUserandDept.bind(_this)()
                    }else{
            ...
            loadFieldValue(){
                ...
                let qs = require('querystring')
                let info = {}
                info.id = this.orderdetailsData.id
                info.label = 9
            ...
            loadfield(){
                ...
                let aval = null
                let bval = null
                if(this.orderdetailsData.id) {
                    aval = this.orderdetailsData.id;
                    bval = this.orderdetailsData.batch_id;
                }
                let data = {
                    label: 9,
                    id: aval,
            ...
            onSubmit(){
                ...
                let data = {
                    "id":this.myform.id,
                    "batch_id":this.orderdetailsData.batch_id,
