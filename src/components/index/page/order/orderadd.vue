<template>
    <div class="orderadd_c">
            ...
            <div v-for="(item,index) in filterList" :key="index" >
                ...
                <el-table-column label="主图" prop="productImg" width="92" v-if="item.prop == 'image' && item.state == 1">
                    <template slot-scope="scope">
                        ...
                        <!-- <img :src="scope.row.productImg" alt="" width="60" height="60" v-else> -->
                        <span v-else> </span>
                    </template>
                </el-table-column>
        ...
        <div class="submit_btn">
            <el-button type="primary" :disabled="isDisable" @click="onSubmit" style="margin-right:50px !important;">保存并审核</el-button>
        ...
        <el-dialog title="选择产品" :visible.sync="dialogVisible" :close-on-click-modal="false" width="90%" class="orderDialog" center>
            ...
            <div class="otherightcontent" style="height:calc(90vh - 150px);margin-bottom:10px">
                ...
                <el-table :data="tableData" border fit @selection-change="selectInfo" style="width:100%">
                    ...
                    <el-table-column prop="remark" label="产品备注" width="130"></el-table-column>
                </el-table>
            </div>
            ...
            onSubmit(){
                ...
                axios({
                    ...
                    if(res.data.code && res.data.code == '200'){
                        ...
                        let info = res.data.map.order
                        _this.$store.state.orderdetailsData = {id:info.id, 'batch_id': info.batch_id};
                        _this.closeTag()
                        _this.$router.push({ path: '/orderDetail' });
                        // if(res.data.map.order){
                        //     _this.submitAdd = false
                        //     _this.myform.id = res.data.map.order.id
                        //     _this.myform.batch_id = res.data.map.order.batch_id
